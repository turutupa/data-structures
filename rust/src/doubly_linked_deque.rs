...
mod test {
    ...
    #[test]
    fn test_create_node() {
        let node = Node::new(35);
        assert_eq!(node.borrow().elem, 35);
    ...
    fn test_push_back() {
        ...
        for i in 0..10 {
            if i > 0 {
                assert_eq!(list.peek_back().unwrap().clone(), i - 1);
                assert_eq!(list.peek_front().unwrap().clone(), 0)
            }
            list.push_back(i);
            assert_eq!(list.peek_back().unwrap().clone(), i);
        }
    }

    // copy pasted from https://rust-unofficial.github.io/too-many-lists/fourth-symmetry.html
    #[test]
    fn basics() {
